...
public final class JwtUtil {
     ...
     * @throws BusinessException if the token is missing, malformed or has no "sub" claim
     */
    public static String extractUserId(String authorizationHeader) {
        ...
        if (parts.length < 2) {
            throw new BusinessException("Authorization token is malformed.");
        }
        ...
        try {
            ...
            if (sub == null || sub.asText().isBlank()) {
                throw new BusinessException("Authorization token does not contain a user id.");
            }
            return sub.asText();
